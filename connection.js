...
module.exports = {
  ...
  registerOS: function registerOS(os, callback) {
    connection.beginTransaction(function(err) {
      ...
      let sql = util.format(
        "INSERT INTO os (NUMERO, DATA_ABERTURA, CLIENTE_ID, PROBLEMA_ID, DETALHES, SITUACAO_ID, PROVEDOR_ID) VALUES (%s, NOW(), %s, %s, '%s', 1, %s)",
        os.number,
        os.clienteId,
        os.problemId,
        os.details,
          ...
          connection.query(sql, function(err, result) {
            ...
            connection.commit(function(err, rows, fields) {
              ...
              /*
                              ...
                              */
              sql = util.format(
                "select cli.nome, prob.titulo from cliente as cli, problema_os as prob where cli.id = %s and prob.id = %s",
                os.clienteId,
                os.problemId
              );
              connection.query(sql, function(err, result) {
